translate <- function(spec_file) {
  ...
  spec <- read_yaml(file.path(pkg_path, spec_file))
  ...
  if (type_df == "grouped_df") grps <- suppressWarnings(group_vars(df))
  if (type_df == "data.frame") row_names <- rownames(df)
  if (type_df != "tibble") df <- as_tibble(df)
  vars <- spec$variables
