...
country_scores <- read_excel(raw_xlsx,
  sheet = 2, # read in data from the second sheet
  skip = 1,
  na = "N/A"
) %>%
...
unique(country_scores$item)
unique(country_scores$sub_item)

country_scores_items <- country_scores %>%
...
unique(country_scores$sub_item)
unique(nchar(country_scores_items$sub_item_description))

country_scores_items %>%
...
country_scores_items <- country_scores_items %>%
  filter(sub_item != "ad")

sort(unique(country_scores_items$sub_item_description))
...
country_scores <- country_scores %>%
  left_join(country_scores_items) %>%
...
# use_data(country_scores_items, overwrite = TRUE)
...
sub_items <- country_scores_items %>%
  distinct(sub_item) %>%
  pull(sub_item)
...
unique(country_rating_texts$sub_item)
