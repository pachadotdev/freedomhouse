...
# recode "-" to missing
country_rating_statuses_23 <- read_excel(raw_xlsx,
  sheet = 2, # read in data from the second sheet
  na = "-"
)

names(country_rating_statuses_23)[1] <- "country"

country_rating_statuses_23

# filter out the first and second row of data
country_rating_statuses_23 <- country_rating_statuses_23 %>%
  filter(
...
countries <- country_rating_statuses_23 %>%
  ungroup() %>%
  distinct(country) %>%
  mutate(
    iso2c = countrycode(
...
saveRDS(country_rating_statuses_23, "dev/country_rating_statuses_23.rds")

# save
country_rating_statuses <- freedomhouse::country_rating_statuses %>%
  bind_rows(country_rating_statuses_23) %>%
  arrange(country, year)

use_data(country_rating_statuses, overwrite = TRUE)

dim(country_rating_statuses)
...
country_scores_23 <- read_excel(raw_xlsx,
  sheet = 2, # read in data from the second sheet
  skip = 1,
  na = "N/A"
) %>%
...
country_scores_23 <- country_scores_23 %>%
  filter(year == 2023)
...
unique(country_scores_23$item)
unique(country_scores_23$sub_item)

country_scores_23_items <- country_scores_23 %>%
...
saveRDS(country_scores_23_items, "dev/country_scores_23_items.rds")
...
unique(country_scores_23$sub_item)
unique(nchar(country_scores_23_items$sub_item_description))

country_scores_23_items %>%
...
country_scores_23_items <- country_scores_23_items %>%
  filter(sub_item != "ad")

sort(unique(country_scores_23_items$sub_item_description))
...
country_scores_23 <- country_scores_23 %>%
  left_join(country_scores_23_items) %>%
...
country_scores <- freedomhouse::country_scores %>%
  bind_rows(country_scores_23) %>%
  arrange(country_territory, year)
...
countries <- str_to_lower(unique(country_rating_statuses_23$country))
...
sub_items <- country_scores_23_items %>%
  distinct(sub_item) %>%
  pull(sub_item)
...
country_rating_texts_23 <- map_df(
...
unique(country_rating_texts_23$country)
unique(country_rating_texts_23$year)

# obtain iso code for the country and then replace with the names as they are in country_rating_statuses_23
...
# country_rating_texts_23_2 <- country_rating_texts_23 %>%
#   mutate(detail_end = str_sub(detail, -20))

# country_rating_texts_23_2 <- country_rating_texts_23_2 %>%
#   distinct(detail_end)

# readr::write_csv(country_rating_texts_23_2, "dev/country_rating_texts_23.csv")
...
detail <- country_rating_texts_23$detail
...
country_rating_texts_23 %>%
  filter(year == 2023, country == "Angola")
...
unique(country_rating_texts_23$sub_item)
...
country_rating_texts <- freedomhouse::country_rating_texts %>%
  bind_rows(country_rating_texts_23) %>%
  arrange(country, year)
